from tkinter import *
from PIL import Image, ImageTk
from random import choice

# Create the main window
w = Tk()
w.title("Rock Paper Scissors")
w.geometry("600x500")

# Load images
image_files = {"Rock": "Rock.jpg", "Paper": "Paper.jpg", "Scissors": "Scissor.png"}
image_list = {key: ImageTk.PhotoImage(Image.open(img).resize((200, 200))) for key, img in image_files.items()}

# Variables to store user choice
user_choice = StringVar()
hide_timer = None

# Function to hide images after 6 seconds
def hide_image():
    computer_label.config(image="")
    user_image_label.config(image="")
    result_label.config(text="")  # Hide result message after 6 seconds

# Function to play the game
def play_game():
    global hide_timer

    user_selection = user_choice.get()
    
    if not user_selection:
        result_label.config(text="Please select Rock, Paper, or Scissors!", fg="red")
        return

    # Computer's random choice
    computer_selection = choice(list(image_files.keys()))

    # Show images in their respective frames
    computer_label.config(image=image_list[computer_selection])
    user_image_label.config(image=image_list[user_selection])

    # Cancel previous timer if still running
    if hide_timer:
        w.after_cancel(hide_timer)

    # Start a new timer to hide the images and result after 6 seconds
    hide_timer = w.after(6000, hide_image)

    # Determine the winner
    if user_selection == computer_selection:
        result = "DRAW"
        color = "blue"
    elif (user_selection == "Rock" and computer_selection == "Scissors") or \
         (user_selection == "Scissors" and computer_selection == "Paper") or \
         (user_selection == "Paper" and computer_selection == "Rock"):
        result = "YOU WIN"
        color = "green"
    else:
        result = "YOU LOSE"
        color = "red"

    result_label.config(text=result, fg=color)

# ** Frame for Computer's Choice **
computer_frame = Frame(w, width=200, height=200, relief="solid", bd=2)
computer_frame.pack(pady=10)
computer_label = Label(computer_frame)
computer_label.pack(expand=True)

# ** Label for Result Message **
result_label = Label(w, text="", font=("Arial", 14, "bold"))
result_label.pack(pady=10)

# ** Frame to Properly Position the Check Button in the Center **
button_frame = Frame(w)
button_frame.pack(expand=True)
button = Button(button_frame, text="Check", command=play_game, font=("Arial", 12))
button.pack()

# ** Frame for User Selection and Image Display **
user_frame = Frame(w)
user_frame.pack(side=BOTTOM, pady=100)

# Frame for user image (fixed size)
user_image_frame = Frame(user_frame, width=200, height=200, relief="solid", bd=2)
user_image_frame.pack()
user_image_label = Label(user_image_frame)
user_image_label.pack(expand=True)

# Label and selection buttons
Label(user_frame, text="Choose:", font=("Arial", 12)).pack()
for option in ["Rock", "Paper", "Scissors"]:
    Radiobutton(user_frame, text=option, variable=user_choice, value=option, font=("Arial", 12)).pack(side=LEFT)

# Run the GUI loop
w.mainloop()
